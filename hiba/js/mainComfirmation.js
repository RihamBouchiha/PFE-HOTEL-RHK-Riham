// Récupérer la dernière réservation de table
fetch('http://localhost:5502/derniere_reservation_table')
  .then(response => response.json())
  .then(data => {
    // Afficher les données dans la page de confirmation
    document.getElementById("nomReservation").textContent = data.nom;
    document.getElementById("prenomReservation").textContent = data.prenom;
    document.getElementById("dateHeureDebutReservation").textContent = data.date_heure_debut;
    document.getElementById("dateHeureFinReservation").textContent = data.date_heure_fin;
    let jour = new Date(data.jour).toLocaleDateString('fr-FR');
    document.getElementById("jourReservation").textContent = jour;
    document.getElementById("typeReservation").textContent = data.type;
    document.getElementById("typeReservation").textContent = data.type;
    
    // Ajouter un message de remerciement personnalisé
    const messageRemerciement = document.getElementById('messageRemerciement');
    messageRemerciement.textContent = `Merci ${data.prenom} ${data.nom} pour votre réservation! Nous avons hâte de vous accueillir à RHK Restaurant.`;
  })
  ...
  document.getElementById("downloadButton").addEventListener("click", function() {
    ...
    sectionContent += `<style>${styles}</style>`; // styles contient tout le CSS que vous avez fourni
    const blob = new Blob([sectionContent], { type: "text/HTML" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = "Votre réservation de table.html";
